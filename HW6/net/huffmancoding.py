            ...
            if 'conv' in name:
                ...
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        ...
                        data_.append(mat.data)
                        lenData_.append(len(mat.data))
                        indices_.append(mat.indices)
                        indptr_.append(calc_index_diff(mat.indptr))
                        test.append(mat.indptr)
                # print('indices_', indices_)
                all_data = np.concatenate(data_)
                all_indices = np.concatenate(indices_)
                all_indptr = np.concatenate(indptr_)
                lenData_ = np.array(lenData_, dtype = np.int32)
                # print('lenData_', lenData_)
                ...
                t3, d3 = huffman_encode(lenData_, name+f'_{form}_len_of_data', directory)
                # t4, d4 = huffman_encode(lenPtr_, name+f'_{form}_len_of_ptr', directory)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                # Decode data
                all_data = huffman_decode(directory, name+f'_{form}_data', dtype='float32')
                all_indices = huffman_decode(directory, name+f'_{form}_indices', dtype='int32')
                # all_indptr = reconstruct_indptr(huffman_decode(directory, name+f'_{form}_indptr', dtype='int32'))
                all_indptr = huffman_decode(directory, name+f'_{form}_indptr', dtype='int32')
                # print('deptr', np.array(all_indptr).shape)
                lenData = huffman_decode(directory, name+f'_{form}_len_of_data', dtype='int32')
                ...
                head_p = 0
                tail_p = 0
                shape1 = [shape[2], shape[3]]
                mats = []
                all_mat = []
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        tail += lenData[i*shape[1]+j] 
                        tail_p += shape[2]
                        data = all_data[head:tail]
                        indices = all_indices[head:tail]
                        indptr = all_indptr[head_p:tail_p]
                        indptr = reconstruct_indptr(indptr)
                        # print('indices', indices, 'len', len(indices))
                        # print('data', len(data))
                        # print('indptr', indptr)
                        mat = matrix((data, indices, indptr), shape1)
                        mat = mat.toarray()
                        mats.append(mat)
                        head = tail
                        head_p = tail_p
                ...
                # print('allmat', np.array(all_mat).shape)
                ...
                param.data = torch.from_numpy(np.array(all_mat)).to(dev)
            elif 'fc' in name:
                form = 'csr' if shape[0] < shape[1] else 'csc'
