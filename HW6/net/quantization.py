        ...
        if len(shape) == 2:  # fully connected layers
            ...
            for i in range(shape[0]):
                for j in range(shape[1]):
                    all_mat.append(weight[i][j])
            all_mat = np.concatenate(all_mat)
            mat = csr_matrix(all_mat)
            ...
            for i in range(shape[0]*shape[1]):
                ...
                for j in range(shape[2]):
                    mat1.append(mat[i*shape[2]+j])
                mat2.append(mat1)

            for i in range(shape[0]):
                mat1 = []
                for j in range(shape[1]):
                    mat1.append(mat2[i*shape[1]+j])
                mat3.append(mat1)

            # Insert to model
            module.weight.data = torch.from_numpy(np.array(mat3)).to(dev)
